...
#Exclude trails.
#Revised 4/17/2020
#Revised 5/29/20 ... making sure you account for all that eco-counter work!!!
...
nrow(aim2_ex_strava_5halfmi_)
setwd(file.path("C:", "Users", "mdg71", "Dropbox", "EMORY",
                "General research",  "Dissertation", "R_data", "analysis data")) 
save(aim2_ex_strava_5halfmi, file = "aim2_ex_strava_5halfmi.RData")

#-----gears by edge exposure lookup-------#######
edge_exposure_lookup = aim2_ex_strava_5halfmi %>% dplyr::select(edge_id, major_or_res) %>% st_set_geometry(NULL)
save(edge_exposure_lookup, file = "edge_exposure_lookup.RData")
...
edge_gears_combined = edge_gears_combined_nogeo %>% left_join(aim2_ex_strava_5halfmi_geolookup, by = "edge_id") %>% st_as_sf()

nrow(gears_by_edge_nogeo)
...
mapview(edge_gears_combined, zcol = "edge_has_gears", color = c("blue", "red"), lwd=4) #excellent.
